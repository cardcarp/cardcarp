...
> Game apps (`apps/ptcg`, `apps/wow`) are live deployments of one shared UI, `apps/template`. Each holds only its `src/core/config.js`, `src/core/storage.js` and deploy files.<br>
...
```bash
git clone https://github.com/cardcarp/cardcarp.git
cd cardcarp
npm install
npm run dev
```
...
| `apps/template/` | `@cardcarp/template` | **Game site UI.** Tabletop and deckbox screens, shared by every game app |
| `apps/ptcg/` | `@cardcarp/ptcg` | **Pokémon TCG.** The template with ptcg's config and storage |
| `apps/wow/` | `@cardcarp/wow` | **World of Warcraft TCG.** The template with WoW's config and storage |
| `apps/relay/` | `cardcarp-relay` | **Multiplayer relay.** Cloudflare Worker + Durable Object echo server synchronizing canvas actions |
...
| `npm run build` | Builds all packages and apps for production | Writes to `./dist` per app; the documentation site to `apps/home/.vitepress/dist` |
...
| `cardcarp-relay` | *(none)* | `npx wrangler deploy --config apps/relay/wrangler.jsonc` |
...
- **`ROOM_TICKET_SECRET`**: Local secret for `apps/relay`. Stored in `apps/relay/.dev.vars` (see `apps/relay/.dev.vars.example`).
