...
// Whether a single filter is currently narrowing results (drives the in-use
// indicator). Resolved against the manifest default, so a filter sitting at a
// default still reads as in use — reprints ship hidden, and the dot says so.
// The reset button is separate: it tracks divergence from the default.
...
// A multi-select with more options than this gets a dialog instead of an
// in-panel control: past a handful of rows the dropdown (combo) or the
// checkbox list (check) is taller than the window, and neither can be
// searched. mtg's Theme is the extreme case at 684 options.
...
// Choices grouped for display: [{ group, options: [{ value, label }] }]
function groups(def) {
    const map = new Map()
...
// --- Selection accessors (read/write into the active dataset's state) ------
// Untouched multi-selects fall back to their manifest default, so it shows as
// selected on load (and returns after a reset) without seeding state.
...
// Unset toggles fall back to their manifest default.
    ...
    .models(class="px-2 pt-4 flex flex-col gap-4")
        ...
        .field(
            ...
            .row(class="relative pl-2 h-10 flex items-center hover:bg-neutral-900 rounded")
                ...
                //- ---- Long multi-select: dialog ----------------------------------
                //- Same trigger as the set filter below, which is the other
                //- multi-select too big to live in the panel.
