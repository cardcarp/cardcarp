<script setup>
// Alignment, offered whenever the selection holds more than one object.
//
// Registered as a contextual section but deliberately NOT keyed to a type. Everything else in
// registry.js answers to one `uiSelection` type; alignment answers to a count, because lining
// things up does not care what they are — three cards, a die and a note, or two playmats. That
// is also the case a type cannot describe: `type` needs every selected node to agree on a tool,
// and a mixed selection has no such agreement (see getToolbarRepresentative). `units` is filled
// in regardless, which is what makes this section possible at all.
//
// "Object" counts a pile once, so a 60-card deck plus a die is two objects and not sixty-one —
// and aligning them moves the deck as one rather than fanning it out along the edge.
...
// Two is the floor: one object is already aligned with itself.
const show = computed(() => units.value >= 2)

const title = computed(() => `Align · ${units.value}`)

// Edge ids match canvas/selection.js. The icons read as the result rather than the gesture:
// a bar on the edge everything is being pulled to, and the shapes that end up against it.
const ROWS = [
    {
        label: 'Horizontal',
        buttons: [
