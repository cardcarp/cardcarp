<script setup>
// Debounced text input for `input`-type filters. Keeps a local value so
// typing stays responsive, and only pushes into the shared search state
// after the user pauses — avoiding a full filter+sort pass per keystroke.
...
// External changes (reset, dataset switch) flow back into the local value.
watch(
    () => props.modelValue,
    (v) => { if (v !== local.value) local.value = v }
)

// Local edits flow up, debounced.
watchDebounced(
    local,
