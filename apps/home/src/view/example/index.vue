<script setup>
// /examples — every project built on the packages, browsable and filterable.
//
// This used to be three routes over two datasets (Deckbuilders / Simulators /
// Resources), parametrised by a `kind`. It is one list now, because the thing
// worth listing turned out to be the PROJECT rather than the half of it you
// happened to arrive from: a game shipped as its own site with both packages
// mounted was appearing twice, once per package, as if it were two things.
//
// So the parameter is gone and the source is fixed (data/game-list.json). The
// browse UI is unchanged — text search plus category and tag facets — and the
// dropdown options are still scrubbed from the data on screen rather than
// declared, so a new tag on a project appears in the filter by itself.
//
// The packages' own documentation is elsewhere: /deckbox and /simulator.
...
// Normalized project items (see view/home/sources.js).
const items = computed(() => sourceItems())

// Scrub a field's distinct values across the dataset into dropdown options,
// shaped as { value: { label } } to match the SelectItem loop and sorted for
// stable order.
...
// Search term lives in ?q= so a filtered view is deep-linkable and survives
// refresh / back-forward. replace() keeps typing out of the history stack.
...
// Text match AND any selected category AND any selected tag. Empty search and
// empty facets don't constrain.
