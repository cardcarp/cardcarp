...
For each game, wherever the app's storage locations say (see [the deckbox's Storage](/deckbox/install#storage)):

```text [locations]
{manifest}             the archive
{sets}                 the set tree
{cards}/{dir}.avif     card art — the one layout that is fixed
```
