<script setup>
// Home shell: the persistent sidebar (part/sidebar.vue — logo + section nav +
// footer, shared with the in-game menu drawer) alongside a <router-view> that
// swaps between the landing page and the collection lists. Active nav state is
// derived from the current route rather than local state, so each section is a
// real, deep-linkable URL.
...
const sidebar_visible = computed(() => is_desktop.value || menu_active.value)
...
.home-view(class="relative p-3 w-dvw h-dvh flex gap-3 bg-black overflow-hidden")
    ...
    .sidebar(
        ...
        :class="sidebar_visible ? 'opacity-100 translate-x-0 visible' : 'opacity-0 translate-x-4 invisible'"
