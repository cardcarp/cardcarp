<script setup>
// Profile management: save this browser's profile to a file, restore one, or clear it.
//
// It lives in this app rather than on cardcarp.com because a profile is per-origin. It is held in
// localStorage (see @cardcarp/core's store/profile.js), which does not cross a subdomain — so the copy
// cardcarp.com could reach was always a different, permanently empty one. The decks this manages are
// written here, by the deckbox and by the table.
//
// Core
...
// This project is one game, so the counts below need no route param — the same constant main.js uses.
const GAME = 'ptcg'

const { profile } = useProfileStore()

// What this browser is actually holding, so the dialog states a fact rather than implying one. A
// profile that has never been written reads as zero and zero, which is the honest answer.
...
onChange(async (files) => {
    ...
    try {
        ...
        // The shape every profile has carried. Without this a file that is valid JSON but something
        // else entirely would replace the profile wholesale and take the decks in it.
    ...
    // Deliberately no navigation afterwards. The version of this on cardcarp.com ended with a
    // router.replace to a route that no longer existed, which threw on every successful restore.
    // Nothing needs to move: the profile is reactive, so the deck lists re-read it where they stand.
})

async function clickClear() {
    if (busy.value) return

    // Asked twice. It is the one control here that destroys something, and there is no undo.
    if (!confirming.value) {
    ...
    DialogPortal
        ...
        DialogContent(asChild @interact-outside="event => event.preventDefault()")
            Motion(
                ...
                //- What is in this browser right now, rather than a claim about what might be.
                ...
                .section(class="px-6 mt-7")
                    ...
                    //- Says what went wrong and what to do, in place, rather than an alert() the
                    //- browser stacks over the dialog.
