...
// The Home tab: hero + "Featured" overview sections. Structurally distinct
// from the collection lists, so it's its own route child rather than a
// parametrised list.
...
// Assign — each carousel needs its own ref/instance; one ref can't drive both.
// const options = { align: 'start', dragFree: false }
// const [deckboxRef] = useEmblaCarousel(options)
// const [simulatorsRef] = useEmblaCarousel(options)

// What each row shows, curated by hand: game keys from data/game-list.json, in
// the order they appear on the page.
//
// Both rows are the same projects seen twice, and that is the point of the
// `kind` each passes to linkProps: a project's root IS its tabletop, so the
// Simulator row lands at the bare domain while the Deckbox row takes the
// /deckbox path into the card archive. Each row's heading links to that
// package's documentation; /examples is where the full project list lives.
const featured = pick([
    'mtg',
    'ptcg',
    'wow',
...
function setTransparentVideoSource() {
  // HEVC alpha is exclusive to Apple WebKit (Safari / iOS)
  const isAppleWebKit =
...
<template lang="pug">
//- `min-w-0` beside the `min-h-0`, matching the article shell: this is a flex item in the
//- shell's row, so its automatic minimum is its CONTENT's minimum rather than zero, and any
//- child too wide to shrink would hold the whole column open and push it off a narrow screen.
//- Nothing on this page is wide enough to do that today — the guard is here so that stays a
//- property of the layout rather than of whatever happens to be listed on it.
ScrollAreaRoot(
    ...
    ScrollAreaViewport(class="relative flex-1 min-h-0 w-full")
        ...
        .lead(class="relative pt-8 xl:pt-18 px-4")
            ...
            .subtitle(class="hidden md:block mt-4 mx-auto text-mauve-500 text-center text-sm lg:text-base xl:text-lg font-extralight") v14.2.0 released on August 26, 2026

        .section(class="mt-10 xl:mt-18")
            .slider-container(
                ref="sliderContainer"
                ...
                .slider-track(
                    ref="sliderTrack"
                    class="relative flex gap-8 w-max hover:[animation-play-state:paused]"
                    :style="{ '--pan-distance': `${maxPan}px` }"
                )
                    img(src="/image/slide.png" class="w-80 sm:w-96 md:w-120 aspect-video shrink-0 object-cover rounded shadow-2xl")
                    img(src="/image/slide.png" class="w-80 sm:w-96 md:w-120 aspect-video shrink-0 object-cover rounded shadow-2xl")
                    img(src="/image/slide.png" class="w-80 sm:w-96 md:w-120 aspect-video shrink-0 object-cover rounded shadow-2xl")
                    img(src="/image/slide.png" class="w-80 sm:w-96 md:w-120 aspect-video shrink-0 object-cover rounded shadow-2xl")
                    img(src="/image/slide.png" class="w-80 sm:w-96 md:w-120 aspect-video shrink-0 object-cover rounded shadow-2xl")
        ...
        .section(class="px-10 mt-10 md:mt-20")
            .lead(class="mx-auto max-w-160 flex justify-center text-white text-xl lg:text-2xl xl:text-5xl text-center font-normal font-stretch-50% leading-snug")
