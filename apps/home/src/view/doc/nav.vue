...
// The slug showing right now. Both of a package's routes land here; the index
// one carries no param at all, which is the empty slug.
const current = computed(() => (route.params.slug ?? '').toString())
    ...
    .section(
        ...
        .pages(class="mt-1.5 flex flex-col")
            router-link(
                ...
                :key="page.slug"
                :to="page.slug ? { name: `${pkg}-page`, params: { slug: page.slug } } : { name: pkg }"
