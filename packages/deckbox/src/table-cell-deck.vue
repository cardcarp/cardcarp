<script setup>
// Table-layout deck row: highlight thumbnail, name, tagline, card total.
//
// `tagline` is the deck's one-line subtitle. Its sibling `description` is the
// long-form [{ title, body }] sections the detail dialog renders — rendering
// that here printed the raw JSON.
...
// Compute
// `props.item` is a deck, not a card: the art belongs to its highlight card,
// which is the record useCardArt wants. Decks with no highlight resolve to
// undefined, which yields no src — the row then shows its fallback icon and
// counts as revealed immediately, with nothing to wait for.
...
.deck(class="relative")
    ...
    //- Skeleton mirrors the real row layout (thumbnail · name/tagline · total)
    //- and fades out once the thumbnail resolves, so images never pop in mid-render.
    Transition(
