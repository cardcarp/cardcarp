...
// Local
//
// One list now, not three tabs: what an accessory is travels with the item as `category`,
// so filtering replaces tabbing. See accessory/list.vue.
...
PopoverRoot(:modal="false" v-model:open="open")
    ...
    PopoverPortal
        AnimatePresence
            //- Nothing is focused on open. The search sits first in this panel's DOM, so Reka's
            //- focus scope would hand it the cursor and a tablet would raise its keyboard over
            //- the list the player opened this to look at. Same as the deckbox: focus stays on
            //- the trigger, and the field is one tap or one Tab away.
            PopoverContent(
                asChild
                align="start"
                side="right"
