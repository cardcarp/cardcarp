...
// Local
// Keep the center buttons aligned with the main area's center column: each
// open side panel is w-80 (320px), which shifts the column's midpoint by half
// that. The shift snaps (no transition) in lockstep with the main grid's
// tracks, which also resize instantly — the panel slide (a composited
// transform) is the only animated part, so nothing janks out of sync.
//
// Only at lg and up, where the panels are columns that take width off the
// center. Below it they don't: the left one overlays and the right one is a
// bottom sheet, so the center column is the full width whatever is open, and
// shifting to follow it would move the buttons off a bar they only just fit.
...
.header(
    ...
    .left(class="flex justify-start items-center gap-2 header-compact")
        ...
        //- The host's chrome. The deckbox owns the slot's place in its header bar; what
        //- goes in it — site navigation, an About dialog, links to the other games — is true of
        //- cardcarp.com and not of a deckbox, so it comes in from outside. Same inversion as
        //- @cardcarp/simulator's #menu slot, for the same reason.
        slot(name="menu")
