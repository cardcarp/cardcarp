...
// Draft — edits live here until the user saves; deck_building only changes on
// commit. Re-seeded from the build each time the dialog opens.
const draft = reactive({
    name: '',
    highlight: '',
    tagline: '',
    // The textarea edits plain text, while the field it writes to is the
    // published [{ title, body }] sections shape. Section 0's body is what a
    // user authors; the flattening happens on commit, not here.
    notes: '',
})

// The build's first note section, and everything after it. A deck copied from
// the archive can carry several sections — the dialog offers one textarea, so
// the rest are preserved rather than edited or dropped.
...
// Whether the draft diverges from the saved build — drives the close prompt.
...
// Apply the draft onto the build, then persist the build to the profile so the
// edited details are recorded (deck_save handles storage / existing / new).
function commit() {
    deck_building.name = draft.name
    deck_building.highlight = draft.highlight
    deck_building.tagline = draft.tagline

    // Keep section 0's title (a copied deck may have one; a user's own is
    // blank) and anything after it. Emptying the text with no other sections
    // present clears the field rather than leaving an empty section behind for
    // the read dialog to filter out again.
...
// Close request from the shell (overlay / escape). With unsaved edits, prompt
// to save or discard; cancel keeps the dialog open.
...
DialogShell(
    ...
    .columns(class="flex-1 min-h-0 flex")
        DialogScroll(
            ...
            //- The one-line subtitle the deck row and the centre detail bar
            //- show. Separate from the notes textarea opposite, which writes
            //- the long-form `description` sections.
            .section(class="mt-6")
                .header(class="flex mb-1.5 px-2 font-mono text-3 tracking-wider opacity-50")
