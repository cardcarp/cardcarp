...
const props = defineProps({
    // 'archive' — the game's published decks, from the manifest.
    // 'saved'   — the player's own decks, from their profile.
    source: {
        type: String,
        default: 'archive'
    }
...
// Assign
// Saved decks come from the profile and need no manifest, so they key off the
// route's game and list immediately; only the archive tab below waits on data.
const { manifest, config, active_game: game_id, loading } = useGameStore()
const { gameDecks } = useProfileStore()

// --- Sources ----------------------------------------------------------------
// Saved decks come straight from the profile, the same records the
// @cardcarp/deckbox's right panel lists — minus Storage, which is the player's whole
// collection rather than something to deal onto the table. Manifest filters
// describe published decks, so they don't apply to these.
const saved_decks = computed(() =>
    gameDecks(game_id.value).filter(deck => deck.id !== 'storage')
)

// Archive decks run through the shared pipeline with every filter the toolbar
// doesn't offer at its manifest default, so the toolbar and @cardcarp/deckbox
// agree on what the archive holds.
//
// `preset.filter` is where the dropdowns below write, so the state has to be
...
// --- Facets -----------------------------------------------------------------
// Format and theme, from the game's own deck vocabulary. Both are optional: a
// game publishes the filters it has, and a dropdown with nothing to choose
// doesn't render (see availableOptions).
//
// That is also what keeps the Saved tab honest without a branch. The player's
// own decks carry neither property, so both resolve to nothing there — which
// is the truth: these describe published decks, and filtering hand-built ones
// by them would empty the list rather than narrow it.
//
// Availability is read off the tab's own unfiltered source, so the archive is
// offered what the archive holds.
...
// Only the archive tab is waiting on anything. The saved tab reads the profile,
// which is local and already keyed to the route's game, so it lists straight
// away and must not be skeletoned alongside it.
const show_skeleton = computed(() => loading.value && props.source !== 'saved')

// The search box narrows the source (fuzzy name match, debounced).
const { searchQuery, filteredData } = useCardSearch(source_decks, 40, 'name')

// --- Infinite scroll --------------------------------------------------------
// Same wiring as the card tab: a cumulative window that grows as the viewport
// nears the end, reading the scrolling element the shared scroll area passes through as
// `viewport` (see part/dialog-scroll.vue).
...
.field-wrap(class="pt-3 pb-2 flex-none")
    .col(class="flex gap-2")
        ...
        //- Two facets share the row when a game publishes both, so each takes half the
        //- panel; one on its own spans it.
