...
// Which level the current route puts us on. A package's index and page routes
// are named `<pkg>` and `<pkg>-page`, so this reads them off the tree rather
// than listing them again here — a third package needs no edit.
const level = computed(() => {
    const pkg = packages.find((p) => route.name === p || route.name === `${p}-page`)
    return pkg ?? 'root'
})
...
const nav = computed(() => ({
    home: { to: { name: 'home' }, label: 'Home', active: route.name === 'home' },
    deckbox: { to: { name: 'deckbox' }, label: 'Deckbox' },
    simulator: { to: { name: 'simulator' }, label: 'Simulator' },
    compile: { to: { name: 'compile' }, label: 'Compile' },
    examples: { to: { name: 'examples' }, label: 'Examples', active: route.name === 'examples' }
}))
...
//- section already showing — that fires no route change.
//-
    ...
    Transition(:name="direction > 0 ? 'level-in' : 'level-out'")
        .level(:key="level" class="flex flex-col gap-1")
            ...
            template(v-else)
                ...
                DocNav(:pkg="level" class="mt-4 px-4")
