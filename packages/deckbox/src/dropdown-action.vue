...
// These dialogs load every card image at once; cap how much they're given.
const PRINT_MAX = 500
const PROBABILITY_MAX = 100

// The cards the actions operate on — the build's list, or the center's
// filtered results (action_card_list in state.js).
...
function click_print() {
    ...
    for (const card of card_list.value) {
        // The same .avif used everywhere else on screen — the print dialog
        // decodes it in the browser and re-encodes to JPEG for the PDF, so the
        // CDN doesn't have to carry a JPEG twin of every card.
        const src = card_src(card)

        // No art means nothing to place on the sheet. Skipping matters here —
        // an empty src would fetch the page itself and put HTML in the PDF.
        if (!src) continue
    ...
    // Only the build has a deck behind it; the center is a filtered slice of
    // the archive, which the dialog falls back to naming after the game.
...
async function click_duplicate() {
    ...
    // Nothing to save from the center when the active dataset has no cards
    // (e.g. the deck archive) — skip the action and its feedback entirely.
    if (prop.location !== 'build' && !card_list.value.length) return

    duplicate_active.value = true
    duplicate_text.value = "Creating..."

    // Build duplicates itself; the center saves its filtered card view as a new deck.
