...
// import Precon from './deckbox/precon.vue'
...
// Stores
//
// Both live in ui.js rather than here: this UI opens the panel on the player's behalf when the
// walkthrough ends, when a returning player arrives, and when the table announces a reset or a
// ?deck= link that found nothing.
import { deckbox_open as popoverDeckbox_open, deckbox_tab as popoverDeckbox_tab } from '../ui.js'

// Every time the player opens the panel themselves it starts on Cards. A tab is somewhere they
// went for one errand, not a preference worth keeping — looking in Saved once should not mean
// finding Saved again next time they reach for a card.
//
// Hung off the popover's own open event rather than a watcher on the shared ref, because only
// the player's own opens (trigger, Escape, click-outside) come through here. openDeckbox in the
// store writes both refs directly and so passes straight by, which is what lets it keep naming
// Decks when the walkthrough ends, a returning player arrives, or the table is reset.
//
// Reset on the way in, never on the way out: changing the tab while the panel is animating shut
// swaps the mounted TabsContent under Reka mid-dismiss, and the panel then never renders again
// — the store reads open, the trigger lights up, and no content mounts.
                ...
                AnimatePresence
                    TooltipContent(asChild align="center" side="right" :sideOffset="6")
                        Motion(
                            ...
                            span Cards
    PopoverPortal
        //- Nothing is focused on open. Reka's focus scope would otherwise land on the first
        //- tabbable thing in the panel, and any path that ends on the search field summons the
        //- on-screen keyboard on a tablet — half the panel covered before the player has asked
        //- to type. Focus stays on the trigger; the field is one tap or one Tab away.
        PopoverContent(
            asChild
            align="start"
            side="right"
