// The game handed to the table, run for real under Node: game.js holds what the host passes as it
// is, never freezes it, lets anything wait on it, and drops a hand dealt from another game.
// Run: node packages/simulator/src/game.test.mjs
...
// A returning player: one seat holding a card, and no game stamp at all — the hand predates
// stamping, so which game it was dealt from cannot be known.
